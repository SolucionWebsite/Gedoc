    ...
    public class BitacoraSrv
    {
        ...
        public ResultadoOperacion ProcesaData()
        {
            ...
            var logId = logRepo.Add(new LogEtl()
            {
                Tipo = "BITACORA-CARGA",
                Fecha = DateTime.Now,
                Descripcion = "CARGA DE DATOS DE BITACORAS"
            });
            #endregion

            #region Fecha de Última Carga
            var fechaUltimaCarga = logRepo.GetFechaUltimaCargaOk("BITACORA-CARGA-FIN");
            if (fechaUltimaCarga == null)
            {
                logRepo.Add(new LogEtl()
                {
                    Tipo = "BITACORA-CARGA-ERROR",
                    Fecha = DateTime.Now,
            ...
            #region SP Carga desde tabla temporal a tabla de bitacoras

            var resultadoSProc = _repo.SpCopiaDatosDest(new { idCarga = logId }, "SpCopiaBitacoras");
            ...
            logRepo.Add(new LogEtl()
            {
                Tipo = "BITACORA-CARGA-FIN" + (esOk ? "" : "-ERROR"),
                Fecha = DateTime.Now,
        ...
        /// <summary>
        /// Carga desde la BD origen las bitácoras entre el rango de fechas especificados y los inserta en la tabla
        /// BitacorassCarga de la BD destino.
        /// </summary>
        ...
        {
            var esOk = true;
            var datos = _repo.GetDatosFromOrigen<Bitacora>(Queries.SelectBitacorasOrigen, new { fechaD = fechaDesde, fechaH = fechaHasta });
            if (datos == null)
            {
                logRepo.Add(new LogEtl()
                {
                    Tipo = "BITACORA-CARGA-ERROR-ORIG",
                    Fecha = DateTime.Now,
            ...
            var datosInsertar = new List<Bitacora>();
            foreach (var dato in datos)
            {
                ...
                if (datosInsertar.Count >= 5000)
                {
                    ...
                    var result = _repo.InsertaEnDestinoTemp(datosInsertar, Queries.InsertBitacoraCarga);
                    if (result.Codigo < 0)
                    {
        ...
        //    var spItems = spClient.GetItemsListaSP("Bitácora", query);
        //    if (spItems == null)
        //    {
        //        logRepo.Add(new LogEtl()
        //        {
        ...
        //        return false;
        //    }
        //    var datos = new List<Bitacora>();
        //    var spclient = new SharePointClient();
        //    var fieldsSp = spclient.GetFieldsFromList("Bitácora");
        //    foreach (var item in spItems)
        //    {
        //        var dato = GenericMap.MapFromSpItemToModel<Bitacora>(item, fieldsSp);
