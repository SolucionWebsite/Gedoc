...
namespace Gedoc.Etl.Winsrv.Servicios
{
    public class LogSistemaSrv
    {
        ...
        /*
         * Migra los datos de LogSistema en la BD principal hacia la tabla TB_Log en la BD sistema
         * Atención. Se cargan los logs q no son Log de Bitácora.
         */
        ...
        public ResultadoOperacion ProcesaData()
        {
            ...
            {
                Tipo = "LOGSISTEMA-CARGA",
                Fecha = DateTime.Now,
                Descripcion = "CARGA DE DATOS DE LOGSISTEMA"
            });
            #endregion

            #region Fecha de Última Carga
            var fechaUltimaCarga = logRepo.GetFechaUltimaCargaOk("LOGSISTEMA-CARGA-FIN");
            if (fechaUltimaCarga == null)
            {
                logRepo.Add(new LogEtl()
                {
                    Tipo = "LOGSISTEMA-CARGA-ERROR",
                    Fecha = DateTime.Now,
            ...
            Logger.Execute().Info($"..... tomando registros modificados/creados a partir de la fecha {fechaDesde:dd/MM/yyyy HH:mm}");
            ...
            #region SP Carga desde tabla temporal a tabla de TB_Log

            var resultadoSProc = _repo.SpCopiaDatosDest(new { idCarga = logId }, "SpCopiaLogSistema");
            ...
            logRepo.Add(new LogEtl()
            {
                Tipo = "LOGSISTEMA-CARGA-FIN" + (esOk ? "" : "-ERROR"),
                Fecha = DateTime.Now,
        ...
        /// <summary>
        /// Carga desde la BD origen los logs entre el rango de fechas especificados y los inserta en la tabla
        /// TB_LogCarga de la BD destino.
        /// </summary>
        ...
        private bool CargaFromOrigenToDestino(int idCarga, DateTime fechaDesde, DateTime fechaHasta)
        {
            ...
            try
            {
                var datos = _repo.GetDatosFromOrigen<LogSistema>(Queries.SelectLogSistemaOrigen, new { fechaD = fechaDesde, fechaH = fechaHasta });
                if (datos == null)
                {
                    logRepo.Add(new LogEtl()
                    {
                        Tipo = "LOGSISTEMA-CARGA-ERROR-ORIG",
                        Fecha = DateTime.Now,
                ...
                var datosInsertar = new List<LogSistema>();
                foreach (var dato in datos)
                {
                    ...
                    if (datosInsertar.Count >= 5000)
                    {
                        ...
                        var result = _repo.InsertaEnDestinoTemp(datosInsertar, Queries.InsertLogSistemaCarga);
                        if (result.Codigo < 0)
                        {
