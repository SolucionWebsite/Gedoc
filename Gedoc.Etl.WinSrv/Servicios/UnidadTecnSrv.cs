    ...
    public class UnidadTecnSrv
    {
        ...
        public ResultadoOperacion ProcesaData()
        {
            ...
            var logId = logRepo.Add(new LogEtl()
            {
                Tipo = "UT-CARGA",
                Fecha = DateTime.Now,
                Descripcion = "CARGA DE DATOS DE UTS"
            });
            #endregion

            #region Fecha de Última Carga
            var fechaUltimaCarga = logRepo.GetFechaUltimaCargaOk("UT-CARGA-FIN");
            ...
            #region SP Carga desde tabla temporal a tabla de UT

            var resultadoSProc = _repo.SpCopiaDatosDest(new { idCarga = logId }, "SpCopiaUnidadesTecn");
        ...
        /// <summary>
        /// Carga desde la BD origen las undades tecnica entre el rango de fechas especificados y los inserta en la tabla
        /// UnidadesTecnicaCarga de la BD destino.
        /// </summary>
        ...
        {
            var esOk = true;
            var datos = _repo.GetDatosFromOrigen<UnidadTecnica>(Queries.SelectUnidadTecnicaOrigen, new { fechaD = fechaDesde, fechaH = fechaHasta });
            if (datos == null)
            {
                logRepo.Add(new LogEtl()
                {
                    Tipo = "UT-CARGA-ERROR-ORIG",
                    Fecha = DateTime.Now,
            ...
            var datosInsertar = new List<UnidadTecnica>();
            foreach (var dato in datos)
            {
                ...
                if (datosInsertar.Count >= 5000)
                {
                    ...
                    var result = _repo.InsertaEnDestinoTemp(datosInsertar, Queries.InsertUnidadTecnicaCarga);
                    if (result.Codigo < 0)
                    {
